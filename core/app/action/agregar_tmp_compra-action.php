...
if (isset($_POST['id'])){$id=$_POST['id'];}//id producto
...
if(isset($_POST['cantidad']) && isset($_POST['precio_venta'])){
    //$producto= TmpData::getByIdProducto($_POST['id']);//este es el q no funciona
    $producto=TmpData::getByIdYSessionYTipo($_POST['id'], $session_id, 2); //2 compras
    ...
    if(count($producto)>0){
        ...
        $temporal->tipo_operacion = 2;//2 es compra, 1 es venta
        $temporal->addTmp();
    }
}
